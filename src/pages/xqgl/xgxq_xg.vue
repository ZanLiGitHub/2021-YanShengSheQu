...
<script>
import {getList, editList, editListByxqID} from "../../network/data";
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      if (this.accept !== true) {
        ...
        //后端通信
        editList('xqList', this.inputs.id, this.inputs)
        editListByxqID('ldList', ...this.ldList)
        editListByxqID('dyList', ...this.dyList)
        editListByxqID('hList', ...this.hList)
        //弹出通知
        this.$q.notify({
          color: "green-4",
          textColor: "white",
  ...
  mounted() {
    //后端通信获取档案
    getList('xqList', {id:this.$route.query.id}).then(res=>{
      //获取原数据
      this.inputs = res[0]
