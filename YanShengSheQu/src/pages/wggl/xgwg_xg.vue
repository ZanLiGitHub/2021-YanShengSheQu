          ...
          <div class="row items-center" style="margin-bottom:30px">
            ...
            <div class="col col-md-2">
              <q-input
                ...
                dense
                :rules="[ val => val && val.length > 0 && val.replace(/\s/g, '').length!==0|| '请输入网格名称']"
              />
            </div>
          </div>
          <!--第二行：网格范围-->
                ...
                <q-card-section>
                  <div class="row items-center">
                    ...
                    <div class="col col-md-8 text-left">
                      <p style="margin: 10px 0 0 70px">楼栋号</p>
                    </div>
                  </div>
                </q-card-section>
                <q-separator/>
                <q-card-section>
                  <div v-for="(item,index) in inputs.xq" :key="index" style="width: 100%;background: white" class="row items-center">
                    <div class="col col-md-4 text-center">
                      {{item}}
                    </div>
                    <div class="col col-md-8 text-left">
...
<script>
import {getList, editList, editListByxqID, addList} from "../../network/data";
...
export default {
  ...
  computed: {
    ldData: function () {
      let ldData=[]
      for(let index=0, length=this.inputs.xq.length; index<length; index++){
        ldData.push([
          {label: '1栋', value: `${index}1`},
          {label: '2栋', value: `${index}2`}
        ])
      }
      return ldData
    },
    ...
    xqmcList: function () {
      ...
      for (let key in this.xqList) {
        list.push({
          label: this.xqList[key].value,
          value: this.xqList[key].id
        })
      }
      return list
    },
  ...
  mounted() {
    //后端通信获取档案
    getList('wgList', {id:this.$route.query.id}).then(res=>{
      //获取原数据
      this.inputs = res[0]
